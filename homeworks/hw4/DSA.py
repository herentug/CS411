...
h1 = pow(g, s1, p)
h2 = pow(g, s2, p)
x=2 # we know the coeffcient between k1 and k2

alfa = (((s1 * h2) - (s2*h1*x)) * modinv((s2 * r1 * x - s1 * r2),q)) % q
print("alfa = " + str(alfa))

if Sig_Ver(message1, r1, s1, public_key_beta, q, p, g):
    print("signature verifies:) for message1 ")
else:
    print("invalid signature:( ")

if Sig_Ver(message2, r2, s2, public_key_beta, q, p, g):
    print("signature verifies:) for message2 ")
else:
    print("invalid signature:( ")
