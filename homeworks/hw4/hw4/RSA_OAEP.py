...
for i in range(0, 10):
    ...
    R =  random.randint(2**(k0-1), 2**k0-1) # R's range defined here
...
# Since we know the encrypted PIN has 4 digits, we can try to find the cipher text
# using brute force on every 4 digit pins. When we find the given cipher text
# that's it. We find the PIN.
...
for R in range(2**7,2**8-1): # R's range that is given above where k0 = 8 and R =  random.randint(2**(k0-1), 2**k0-1)
  for PIN in range(1000,10000):   # there is actually 9999 numbers between 1000 - 9999
     cipher_candidate = RSA_OAEP_Enc(PIN, e, N, R)
     if cipher_candidate == c:
       print("R is ",R,"PIN is", PIN)
       break