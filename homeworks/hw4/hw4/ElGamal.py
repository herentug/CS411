...
# In the encryption function, k is given a random integer between 1 and 2**16 - 1, shown below.
# k = random.randint(1, 2**16-1)
# r = pow(g, k, p)
# t = (pow(h, k, p) * m) % p
k = 0
for i in range(1, 2**16 - 1):
    candidate_r = pow(g,i,p)
    if(r == candidate_r):
        k = i
        break
...
inverse_h = modinv(h**k, p)

print("Inverse of h mod p = " + str(inverse_h))

# in order to get the message we have to multiply inverse_h with t in mod p

m = (inverse_h * t) % p
print("Message is = " + str(m))

# then to get the plaintext convert it to bytes and print it
